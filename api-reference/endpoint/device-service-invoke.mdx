...
const serviceResult = await sendServiceData(token, rebootService);
...
result = send_service_data(token, reboot_service)
...
<Info>
**服务缓存**: 当 `isCache` 为 true 时，服务调用会被缓存。设备上线后会自动执行缓存的服务调用。
</Info>

<Warning>
**缓存时间**: 缓存时间到期后，未执行的服务调用将被自动清除。合理设置 `cacheTime` 以平衡存储成本和执行机会。
</Warning>
