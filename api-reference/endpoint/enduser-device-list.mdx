---
title: '查询用户设备列表'
openapi: 'GET /v2/binding/enterpriseapi/userDeviceList'
---
...
`GET /v2/binding/enterpriseapi/userDeviceList`
...
async function getUserDeviceList(token, uid, page = 1, pageSize = 10) {
  const response = await fetch(
    `https://iot-api.quectelcn.com/v2/binding/enterpriseapi/userDeviceList?uid=${uid}&page=${page}&pageSize=${pageSize}`,
    {
      method: 'GET',
      headers: {
        'Authorization': token,
...
// 获取用户设备列表
const deviceList = await getUserDeviceList(token, 'user123', 1, 20);
